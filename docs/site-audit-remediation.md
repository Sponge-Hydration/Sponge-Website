...
- **Support page:** `/support` — setup, order tracking, cancelling a pre-order, returns, warranty claims, privacy requests, quick checks drawn only from the setup guide, and contact details. Every promise restates an existing policy and links to it. Linked first in the footer's Support column and from the Contact page; the Privacy Policy is now in the Support column too.
- **Review:** a four-lens independent review (Stripe API correctness against Stripe's docs, claims vs policies, wiring/regressions, assent enforceability) with two adversarial skeptics per finding confirmed **no** defects; four low-cost improvements from it were applied anyway (tutorial wording, the 14-day lost-package window, the business name in the checkbox text, the Terms version on each order).
- **Guard:** `test/checkout-terms.test.js` — the checkbox is required and links the Terms; a terms-setup rejection falls back to a working session with the same order; an unrelated Stripe error does not retry.
- **Status:** **Complete**

### A-67 — Full Terms of Service, presented at checkout
- **Request:** Nathan, 2026-09-23: a Terms of Service under Support in the footer, "definitely in favor of the company" and protective against liability. The old `/legal/terms` was three sentences, linked only from the footer's bottom row.
- **What shipped:** 25 sections — not-medical-advice and assumption of risk; measurements are estimates; App Lock risk (never lock an app you need for safety, work or caregiving; no guarantee it locks or unlocks); sharing/caregiving features are not a monitoring or alert service; beta and in-development products; orders (right to refuse or cancel with refund, pricing errors, resale limits, chargebacks); title and risk of loss pass at the carrier; product safety (magnets, swallowing, battery, water, adhesive); software licence and restrictions; conspicuous "as is" disclaimer with implied warranties limited to the 1-year warranty term; exclusion of consequential damages and a cap at the greater of the amount paid or $50; indemnity; review and feedback licence with FTC-safe moderation ("we do not decline to publish a review because it is negative"); binding individual AAA consumer arbitration with 60-day informal resolution, small-claims and IP carve-outs, class-action and jury waiver, batch handling for 25+ coordinated claims, a public-injunctive-relief severance, and a 30-day opt-out; California law with Orange County courts; one-year claims limit; Apple's required App Store terms; IP and copyright notices; changes and termination; the California Civil Code §1789.3 notice.
- **Kept consistent on purpose:** Section 1 says the Pre-Order, Return and Warranty policies control on their subjects, so the terms cannot undercut the cancel-anytime pre-order right, the 30-day money-back guarantee or the 1-year warranty — a contradiction there would invite a deceptive-practices claim rather than prevent one. The company address is not published (it is a residence); legal notices go to team@ with "Legal Notice" in the subject.
- **Presented to the buyer:** `/checkout` now says, directly above "Continue to secure checkout", that continuing means agreeing to the Terms including arbitration and the class waiver, with links. Terms buried only in a footer are often unenforceable. Footer Support column links "Terms of Service".
- **Guard:** `test/terms.test.js` fails if a protective clause disappears, if the consumer promises stop matching the other policies, if the checkout notice moves below the button or loses the arbitration mention, or if the Support link goes.
- **Open, needs Nathan:** have a lawyer review before relying on it, Section 17 above all. Optional hardening: turn on Stripe Checkout's required terms checkbox (`consent_collection[terms_of_service]`) — it needs the terms URL set in the Stripe dashboard first, or every checkout will fail.
