...
if (res.status === 503) {
  verdict('NOT CONNECTED - credentials are missing', red, [
    ...
    '',
    b('Fix:') + ' Cloudflare -> Workers & Pages -> sponge-website ->',
    '  Settings -> Environment variables -> Production.',
    '',
    '  AIRTABLE_BASE_ID and AIRTABLE_REVIEWS_TABLE are already set.',
    '  The only one still missing is:',
    '    ' + b('AIRTABLE_API_KEY') + '  an Airtable personal access token with',
    '                      data.records:read + data.records:write, granted',
    '                      access to the Sponge Feedback base specifically.',
    '',
